...
| 4 Accept集成 | 精确审阅包、锁内核对、基线对齐复验、新Accept、逐仓journal恢复                                                                                          | Agent 阶段沙箱（澄清与判定）与 Accept/集成目前仅 macOS 开通，Linux 不放宽 |
...
7. Linux 平台：执行期隔离使用 bubblewrap（见[多仓执行方案 §12](issue-workspace-execution.md#12-第一版实现与操作边界)），需要可用的 user namespace；**Agent 阶段沙箱（澄清与判定）目前仅实现 macOS**（非 macOS 直接拒绝，不回退无隔离执行），受控本地 HTTP 目标与 Accept/集成同样仅在 macOS 开通，未验证平台保持拒绝。
